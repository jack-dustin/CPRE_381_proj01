...
-- (some) Branch inst. need c_in & c_out of MSB
    -- Need an addSub that has those as ports

entity addSub_BRANCH is
...
architecture structural of addSub_BRANCH is
    ...
    component basic_adder_n_BRANCH is
        generic (N : integer := 32);     -- 4 by default, but allowed to change
    ...
    begin   
        ...
        INST_ADDER: basic_adder_n_BRANCH
            generic map (N => N)
            port map(i_aB0      => i_Da,
                     i_aB1      => Mux_t_Add,
                     o_S        => o_Sum,
